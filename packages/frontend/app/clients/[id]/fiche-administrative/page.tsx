...
import { getDossierById, updateDossier, createDossier } from '@/lib/api/dossiers';
import { toast } from 'react-hot-toast';
import { 
  Save, ArrowLeft, CheckCircle, AlertCircle, User, 
  Mail, Phone, MapPin, FilePlus, Layout, Clock,
  Info, Check, X, ChevronRight, ChevronDown, FileText,
  Download, Eye, ArrowRight, HelpCircle, Calendar,
  Shield, FileCheck
} from 'lucide-react';
import DashboardLayout from '@/components/layout/DashboardLayout';

export default function FicheAdministrativePage() {
  ...
  // Types de dossiers disponibles avec description
  const typesDossiers = useMemo(() => [
  ...
  // Toujours garder les anciennes listes pour la compatibilité avec le composant existant
  const etatsDossier = [
    "NOUVEAU", "EN_COURS", "EN_ATTENTE", "TERMINE", "ARCHIVE", "REFUSE", "ATTENTE_PIECES"
  ];
  ...
  // Charger les données du client et du dossier
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Récupérer le dossier associé si disponible
        if (clientData.dossierId) {
          try {
            const dossierResponse = await getDossierById(clientData.dossierId);
            const dossierData = dossierResponse.data;
            ...
            setFormData(prevData => ({
              ...
              surface: dossierData.surfaceProjet?.toString() || '',
              parcelleCadastrale: dossierData.projectAddress?.cadastralReferences || '',
              zoneUrbanisme: dossierData.zoneUrbanisme || '',
              plu: dossierData.plu || '',
              abf: dossierData.abf || 'NON',
              dateDepot: dossierData.dateDepot || '',
              delaiInstruction: dossierData.delaiInstruction?.toString() || '1',
              dateValidite: dossierData.dateValidite || '',
              commentairesInternes: dossierData.commentairesInternes || '',
              documentsManquants: dossierData.documentsManquants || '',
              avisUrbanisme: dossierData.avisUrbanisme || '',
              etatDossier: dossierData.status || 'EN_COURS',
              priorite: dossierData.priority || 'NORMAL',
              montantFacture: dossierData.montantFacture?.toString() || '',
              acompteRecu: dossierData.acompteRecu?.toString() || '',
              dateAcompte: dossierData.dateAcompte || '',
              soldeRecu: dossierData.soldeRecu?.toString() || '',
              dateSolde: dossierData.dateSolde || '',
            }));
          } catch (error) {
            console.error('Erreur lors de la récupération du dossier:', error);
            toast.error('Le dossier associé n\'a pas pu être chargé.');
          }
        }
      } catch (error) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => {
      ...
      if (name === 'montantFacture' || name === 'acompteRecu' || name === 'soldeRecu') {
        const montantFacture = parseFloat(newFormData.montantFacture) || 0;
        const acompteRecu = parseFloat(newFormData.acompteRecu) || 0;
        const soldeRecu = parseFloat(newFormData.soldeRecu) || 0;
        const montantRestant = montantFacture - acompteRecu - soldeRecu;
        
        newFormData.montantRestant = montantRestant.toString();
      }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!dossier) {
      toast.error('Aucun dossier associé à mettre à jour.');
      return;
    }
    ...
    try {
      ...
      // Préparer les données à mettre à jour sans utiliser de cast explicite
      // Nous allons utiliser les valeurs directement, car elles doivent correspondre aux valeurs des énumérations
      const dossierUpdate = {
        // Les valeurs correspondent aux enum définis dans le type DossierType
        type: formData.dossierType,
        surfaceProjet: parseFloat(formData.surface) || 0,
        zoneUrbanisme: formData.zoneUrbanisme,
        plu: formData.plu,
        abf: formData.abf,
        dateDepot: formData.dateDepot,
        delaiInstruction: parseInt(formData.delaiInstruction) || 1,
        dateValidite: formData.dateValidite,
        commentairesInternes: formData.commentairesInternes,
        documentsManquants: formData.documentsManquants,
        avisUrbanisme: formData.avisUrbanisme,
        // Ces valeurs correspondent aux enum définis
        status: formData.etatDossier,
        priority: formData.priorite,
        montantFacture: parseFloat(formData.montantFacture) || 0,
        acompteRecu: parseFloat(formData.acompteRecu) || 0,
        dateAcompte: formData.dateAcompte,
        soldeRecu: parseFloat(formData.soldeRecu) || 0,
        dateSolde: formData.dateSolde,
        projectAddress: {
          ...(dossier.projectAddress || {}),
          cadastralReferences: formData.parcelleCadastrale,
        },
      };
      
      // Mettre à jour le dossier avec un cast pour satisfaire TypeScript
      // car dossierUpdate ne correspond pas parfaitement au type attendu
      await updateDossier(dossier.id, dossierUpdate as any);
      
      // Notifier de la mise à jour
      toast.success('Fiche administrative mise à jour avec succès');
      
      // Déclencher l'événement de mise à jour pour rafraîchir les autres vues
  ...
  // Recommandation algorithmique du type de dossier
  const getRecommendedDossierType = useCallback(() => {
    if (!client) return 'DP';
    
    // Logique simplifiée basée sur les informations du dossier ou du client
    const travaux = dossier?.description?.toLowerCase() || '';
    const surfaceExistante = dossier?.surfaceExistant || 0;
    const surfaceProjetee = dossier?.surfaceProjet || 0;
    const surfaceCreee = surfaceProjetee - surfaceExistante;
    
    if (travaux.includes('fenêtre') || travaux.includes('fenetre') && !travaux.includes('façade')) {
      return 'DP_FENETRE';
    } else if (surfaceCreee > 40) {
      return 'PC+RT';
    } else if (travaux.includes('isolation') || travaux.includes('ite')) {
      return 'DP_ITE';
    } else if (travaux.includes('façade') || travaux.includes('facade')) {
      return 'DP';
    } else {
      return 'DP';
    }
        ...
        <div className="p-6">
          <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg">
            <div className="flex items-start">
              ...
              <div>
                <h3 className="font-semibold">Client non trouvé</h3>
  ...
  // Formater les données client pour les afficher
  const clientData = {
    id: client.reference || `CL-${new Date().getFullYear()}-XXX`,
    nom: `${client.contactInfo?.firstName || ''} ${client.contactInfo?.lastName || ''}`,
    email: client.contactInfo?.email || '',
    telephone: client.contactInfo?.phone || '',
    adresse: client.addresses && client.addresses.length > 0 
      ? `${client.addresses[0].street}, ${client.addresses[0].postalCode} ${client.addresses[0].city}` 
      : '',
    type: client.clientType || 'Non spécifié',
    creation: new Date(client.createdAt).toLocaleDateString('fr-FR'),
    dernierContact: client.lastContactDate 
      ? new Date(client.lastContactDate).toLocaleDateString('fr-FR')
      : new Date(client.createdAt).toLocaleDateString('fr-FR')
  };
  
  // Données projet (simulées ou à partir du dossier)
  const projetData = dossier ? {
    adresse: dossier.projectAddress ? 
      `${dossier.projectAddress.street || ''}, ${dossier.projectAddress.postalCode || ''} ${dossier.projectAddress.city || ''}` : 
      clientData.adresse,
    surfaceExistante: dossier.surfaceExistant || 0,
    surfaceProjetee: dossier.surfaceProjet || 0,
    changementDestination: dossier.changementDestination || false,
    travaux: dossier.description ? dossier.description.split(',').map(t => t.trim()) : [],
    description: dossier.description || 'Aucune description disponible'
  } : {
  ...
  return (
    <DashboardLayout activeMenu="clients">
      <div className="bg-gray-50 min-h-screen p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
              ...
              <Link href={`/clients/${clientId}`} className="text-blue-500 hover:text-blue-600">{clientData.nom}</Link>
              <ChevronRight size={14} />
          ...
          <div className="lg:col-span-1">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-6">
              <div className="border-b border-gray-200 bg-gray-50 px-4 py-3 flex justify-between items-center">
                ...
                <Link 
                  href={`/clients/${clientId}/fiche-projet`} 
              ...
              <div className="p-4">
                <div className="space-y-4">
                  ...
                  {projetData.travaux && projetData.travaux.length > 0 && (
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                          <div className="border border-gray-200 rounded-lg p-4 bg-white">
                            ...
                            <p className="text-sm text-gray-600 mb-3">
                              {typesDossiers.find(t => t.id === formData.dossierType)?.description || 'Aucune description disponible'}
                            </p>
                            <div className="space-y-2">
                              ...
                              {formData.dossierType.includes('PC') && (
                                <div className="flex items-center gap-2 text-sm">
                                  <Check size={14} className="text-green-500" />
                                  <span className="text-gray-700">Étude thermique RE2020 incluse</span>
                                </div>
                              )}
                              {formData.dossierType.includes('SIGNATURE') && (
                                <div className="flex items-center gap-2 text-sm">
                                  <Check size={14} className="text-green-500" />
                            ...
                            <div className="space-y-3">
                              <div>
                                ...
                                <p className="text-sm font-medium text-gray-700">
                                  {formData.dossierType.includes('PC') ? '15-20 jours ouvrés' : '10-15 jours ouvrés'}
                                </p>
                              </div>
                              <div>
                                <p className="text-xs text-gray-500 mb-1">Délai d'instruction mairie</p>
                                <p className="text-sm font-medium text-gray-700">
                                  {formData.dossierType.includes('PC') ? '2-3 mois' : '1 mois'}
                                </p>
                              </div>
                            </div>
                          </div>
                            ...
                            {etapesList.map((etape) => (
                              ...
                              >
                                <input
                                  ...
                                  name="etatDossier"
                                  value={etape.id}
                                  checked={formData.etatDossier === etape.id}
                                  onChange={handleChange}
                                  className="mr-2"
                                />
                          ...
                          <div className="space-y-2">
                            {statutsList.map((statut) => (
                              <label 
                                key={statut.id}
                                className={`flex items-center p-3 border rounded-lg cursor-pointer ${
                                  formData.etatDossier === statut.id 
                                    ? 'bg-blue-50 border-blue-300' 
                                    : 'bg-white border-gray-200 hover:bg-gray-50'
                                }`}
                              >
                                <input
                                  ...
                                  name="etatDossier"
                                  value={statut.id}
                                  checked={formData.etatDossier === statut.id}
                                  onChange={handleChange}
                                  className="mr-2"
                                />
                                <span className={`
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          >
                            <input
                              ...
                              name="priorite"
                              value="URGENT"
                              checked={formData.priorite === 'URGENT'}
                              onChange={handleChange}
                              className="mr-2"
                            />
                          ...
                          >
                            <input
                              ...
                              name="priorite"
                              value="NORMAL"
                              checked={formData.priorite === 'NORMAL'}
                              onChange={handleChange}
                              className="mr-2"
                            />
                          ...
                          >
                            <input
                              ...
                              name="priorite"
                              value="FAIBLE"
                              checked={formData.priorite === 'FAIBLE'}
                              onChange={handleChange}
                              className="mr-2"
                            />
                      ...
                      <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-3">
                            <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                              ...
                              <span className="text-sm font-medium text-gray-800">
                                {formData.dossierType ? typesDossiers.find(t => t.id === formData.dossierType)?.label || formData.dossierType : '-'}
                              </span>
                            </div>
                            <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                              <span className="text-sm text-gray-500">Étape</span>
                              <span className="text-sm font-medium text-gray-800">
                                {etapesList.find(e => e.id === formData.etatDossier)?.label || formData.etatDossier}
                              </span>
                            </div>
                            <div className="flex justify-between items-center pb-2 border-b border-gray-100">
                              <span className="text-sm text-gray-500">Statut</span>
                              <span className="text-sm font-medium text-gray-800">
                                {statutsList.find(s => s.id === formData.etatDossier)?.label || formData.etatDossier}
                              </span>
                            </div>
                            <div className="flex justify-between items-center">
                              <span className="text-sm text-gray-500">Priorité</span>
                              <span className={`text-sm font-medium ${
                                formData.priorite === 'URGENT' ? 'text-red-600' :
                                formData.priorite === 'NORMAL' ? 'text-amber-600' : 'text-green-600'
                              }`}>
                                {formData.priorite === 'URGENT' ? 'Haute' : 
                                 formData.priorite === 'NORMAL' ? 'Normale' : 'Basse'}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="border border-gray-200 rounded-lg p-4 bg-white">
                          ...
                          <div className="space-y-3">
                            ...
                            {formData.dossierType && formData.dossierType.includes('PC') && (
                              <div className="flex items-start gap-2">
                                <div className="flex items-center justify-center mt-0.5 h-5 w-5 rounded-full bg-blue-100 text-blue-700 text-xs font-medium">
                                  4
                                </div>
        ...
        {/* Toast de succès - géré par react-hot-toast */}
      </div>
    </DashboardLayout>
  );
}